  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "user",
          content: input ? input : "what is 2+2?, output as json with keys input and output",
        },
      ],
    });

    //if response is json parse it else return message
    completion ? console.log(completion.choices[0].message) : res.status(500).json({ error: "No response from OpenAI" });
    ...
    if(!isJSON(content)) {
        // content is not json
        return res.status(200).json({ aiMessage: completion.choices[0].message, response: { output: content } });
    }
        else {
        // content is json

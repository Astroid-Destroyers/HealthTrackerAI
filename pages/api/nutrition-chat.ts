import type { NextApiRequest, NextApiResponse } from 'next';
import { openai } from '@/utils/openai';

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse
) {
  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Method not allowed' });
  }

  try {
    const { message, remainingMacros } = req.body;

    if (!message) {
      return res.status(400).json({ error: 'Message is required' });
    }

    const systemPrompt = `You are a nutrition assistant helping users meet their daily macro goals. 
${remainingMacros ? `The user has the following macros remaining for today:
- Calories: ${remainingMacros.calories} kcal
- Protein: ${remainingMacros.protein}g
- Carbs: ${remainingMacros.carbs}g
- Fats: ${remainingMacros.fats}g` : ''}

When recommending foods:
1. First provide a brief explanation of your reasoning (2-3 sentences max)
2. Then suggest specific foods or meals that fit their remaining macros
3. Provide estimated macro values for each suggestion (calories, protein, carbs, fats)
4. Be concise and practical
5. Include portion sizes when relevant

IMPORTANT: You MUST respond with ONLY a valid JSON object in this exact format, with no additional text:
{
  "explanation": "Based on your remaining macros, I'm focusing on high-protein foods that keep you under your calorie goal. These options will help you hit your protein target without exceeding your daily limits.",
  "foods": [
    {
      "name": "Grilled Chicken Breast (6oz)",
      "calories": 280,
      "protein": 53,
      "carbs": 0,
      "fats": 6
    },
    {
      "name": "Brown Rice (1 cup)",
      "calories": 216,
      "protein": 5,
      "carbs": 45,
      "fats": 2
    }
  ]
}

Do not include any text outside the JSON object. Only valid JSON.`;

    const completion = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: message }
      ],
      temperature: 0.7,
      max_tokens: 500,
    });

    const reply = completion.choices[0]?.message?.content || '[]';

    // Try to parse as JSON, if it fails, return as text
    try {
      const response = JSON.parse(reply);
      if (response.foods && response.explanation) {
        res.status(200).json({ 
          foods: response.foods, 
          explanation: response.explanation,
          isStructured: true 
        });
      } else {
        // Fallback to text response if not valid structure
        res.status(200).json({ reply, isStructured: false });
      }
    } catch {
      // Fallback to text response if not valid JSON
      res.status(200).json({ reply, isStructured: false });
    }
  } catch (error: any) {
    console.error('OpenAI API Error:', error);
    res.status(500).json({ 
      error: 'Failed to get AI response',
      details: error.message 
    });
  }
}
